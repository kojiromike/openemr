...
class AppointmentService extends BaseService
{
   ...
   */
    ...
    {
        // Generated by GitHub Copilot - Using Carbon for date/time operations
        use OpenEMR\Common\Utils\DateHelper;
        use Carbon\Carbon;
        
        $startCarbon = DateHelper::parse($data['pc_startTime']);
        $startTime = $startCarbon->format('H:i:s');

        // Add duration in seconds to get end time
        $endTime = $startCarbon->copy()->addSeconds($data['pc_duration']);
        ...
        if ($GLOBALS['select_multi_providers']) {
            ...
            if ($recurr_affect == 'current') {
                ...
                foreach ($providers_current as $provider) {
                    ...
                    // Generated by GitHub Copilot - Using Carbon for date formatting
                    use OpenEMR\Common\Utils\DateHelper;
                    
                    $selected_date = DateHelper::formatToSqlDate($event_selected_date);
                    // End of AI-generated code
                ...
                // Generated by GitHub Copilot - Using Carbon for date arithmetic
                $selected_date = DateHelper::subDays($event_selected_date, 1)->format('Y-m-d');
                // End of AI-generated code
                foreach ($providers_current as $provider) {
            ...
            if ($recurr_affect == 'current') {
                ...
                $oldRecurrspec = unserialize($origEvent['pc_recurrspec'], ['allowed_classes' => false]);
                // Generated by GitHub Copilot - Using Carbon for date formatting
                $selected_date = DateHelper::parse($_POST['selected_date'])->format('Ymd');
                // End of AI-generated code
                if ($oldRecurrspec['exdate'] != "") {
                    $oldRecurrspec['exdate'] .= "," . $selected_date;
                } else {
                ...
                // Generated by GitHub Copilot - Using Carbon for date arithmetic
                $selected_date = DateHelper::subDays($_POST['selected_date'], 1)->format('Ymd');
                // End of AI-generated code
                sqlStatement("UPDATE openemr_postcalendar_events SET " .
                    " pc_enddate = ? " .
