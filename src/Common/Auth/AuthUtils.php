...
class AuthUtils
{
    ...
    private function checkPasswordNotExpired($user)
    {
        ...
        if ((!empty($query)) && (!empty($query['last_update_password'])) && (check_integer($GLOBALS['password_expiration_days'])) && (check_integer($GLOBALS['password_grace_time']))) {
            ...
            $current_date = DateHelper::formatToSqlDate();
            $totalDays = (int)$GLOBALS['password_expiration_days'] + (int)$GLOBALS['password_grace_time'];
            $expiredPlusGraceTime = DateHelper::addDays($query['last_update_password'], $totalDays)->format('Y-m-d');
            
            if (DateHelper::isGreaterThan($current_date, $expiredPlusGraceTime)) {
