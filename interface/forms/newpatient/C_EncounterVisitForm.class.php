...
class C_EncounterVisitForm
{
     ...
     * @throws \Exception
     */
    public function __construct(string $templatePath, Kernel $kernel, array $issueTypes)
    {
        // Initialize Twig
        $twigContainer = new TwigContainer($templatePath . '/templates/', $GLOBALS['kernel']);
        $this->issueTypes = $issueTypes;
        $this->twigEnvironment = $twigContainer->getTwig();
        // add a local twig function so we can make this work properly w/o too many modifications in the twig file
        $this->twigEnvironment->addFunction(new TwigFunction('displayOptionClass', [$this, 'displayOption']));
    ...
    public function displayOption($field): void
    {
    ...
    public function getCareTeamFacilityForPatient($pid): ?string
    {
...
// Get providers list
    /**
     * @return list<array{id: mixed, name: non-falsy-string, selected: bool}>
     */
    public function getProvidersForTemplate(UserService $userService, array $encounter): array
    {
...
// Get facilities list
    public function getFacilitiesForTemplate(FacilityService $facilityService, $default_fac_override)
    {
        ...
        $facilities = array_map(function (array $facility) use ($default_fac_override): array {
            return [
                'id' => $facility['id'],
                'name' => $facility['name'],
...
// START AI GENERATED CODE
    /**
     * @return list<array{id: mixed, name: mixed, selected: bool}>
     */
    public function getBillingFacilityForTemplate(FacilityService $facilityService, $default_bill_fac = null): array
    {
        ...
        if (empty($default_bill_fac)) {
            ...
            if (!empty($user_facility) && $user_facility['billing_location'] == '1') {
                ...
                $default_bill_fac = empty($tmp_be['id']) ? (!empty($tmp_bl['id']) ? $tmp_bl['id'] : null) :
                    ($tmp_be['id']);
            }
        }
        ...
        foreach ($fres as $fre) {
            $billingFacilities[] = [
                'id' => $fre['id'],
                'name' => $fre['name'],
                'selected' => ($fre['id'] == $default_bill_fac)
            ];
        }
...
// END AI GENERATED CODE
     ...
     * @return list<array{id: mixed, name: mixed, selected: bool}>
     */
    public function getVisitCategoriesForTemplate($viewmode, array $encounter, $default_visit_category): array
    {
        ...
        $categories = [];
        $recordset = sqlStatement($visitSQL);
...
// Get sensitivity options
    /**
     * @return array{value: mixed, display: mixed, selected: bool}[]
     */
    public function getSensitivitiesForTemplate(array $encounter): array
    {
        ...
        foreach ($sensitivities as $sensitivity) {
            ...
            $options[] = [
                'value' => $sensitivity[1],
                'display' => $sensitivity[1],
                'selected' => ($viewmode && $encounter['sensitivity'] == $sensitivity[1])
            ];
        }
...
// Get issues for linking
    /**
     * @return list<array{id: mixed, type: mixed, title: mixed, date: mixed, selected: bool}>
     */
    public function getIssuesForTemplate($pid, $viewmode, $encounter_id, $selectedIssue = null): array
    {

        $issues = [];
        $recordset = sqlStatement("SELECT id, type, title, begdate FROM lists WHERE " .
    ...
    /**
     * @return list<array{option_id: mixed, title: mixed, selected: bool}>
     */
    public function getDischargeDispositionsForTemplate($encounter = null): array
    {
        ...
        $listService = new ListService();
        $dispositionList = $listService->getOptionsByListName('discharge-disposition') ?? [];
    ...
    /**
     * @return list<array{code: mixed, title: mixed, selected: bool}>
     */
    public function getPosOptionsForTemplate($facilityPosCode = null): array
    {
        $posRef = new \POSRef();
        $options = [];
        foreach ($posRef->get_pos_ref() as $pos) {
            $options[] = [
                'code' => $pos['code'],
                'title' => $pos['title'],
     ...
     */
    public function render($pid): void
    {


// GENERATED BY claude.ai January 30th 2025 -- FOOTER

        $mode = (empty($_GET['mode'])) ? 'new' : $_GET['mode'];
...
// under this mode the facility and the visit category will be same as the origin and in readonly
        if ($mode === "followup") {
            $encounter = (empty($_GET['enc'])) ? null : (int)$_GET['enc'];
        ...
        if ($viewmode) {
            ...
            if ($encounter['sensitivity'] && !AclMain::aclCheckCore('sensitivities', $encounter['sensitivity'])) {
                $this->twigEnvironment->render("newpatient/unauthorized.html.twig");
                exit();
            }
        }
        $posCode = '';
...
// Prepare data for template
        ...
        $issuesAuth = true;
        foreach (array_keys($this->issueTypes) as $type) {
...
//Note that for technical reasons, we are bypassing the standard validateUsingPageRules() call.
        ...
        $miscBillingOptions = new MiscBillingOptions();
        $refProviderId = QueryUtils::fetchSingleValue(
            "SELECT ref_providerID FROM patient_data WHERE pid = ?",
            'ref_ProviderID',
            [$pid]
        );
        $referringProviders = array_map(function (array $provider) use ($viewmode, $encounter, $refProviderId): array {
            ...
            return $provider;
        }, $miscBillingOptions->getReferringProviders());

        $orderingProviders = array_map(function (array $provider) use ($viewmode, $encounter, $pid) {
            $provider['selected'] = $provider['id'] == ($encounter['ordering_provider_id'] ?? 0);
            return $provider;
        }, $miscBillingOptions->getOrderingProviders());
...
// Render template
        echo $this->twigEnvironment->render($event->getTwigTemplate(), $event->getTwigVariables());
    }
