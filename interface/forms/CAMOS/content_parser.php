...
function addAppt(string $days, string $time): int
{
    ...
    $escapedDays = add_escape_custom($days);
    $sql = "insert into openemr_postcalendar_events (pc_pid, pc_eventDate," .
    "pc_comments, pc_aid,pc_startTime) values (?, date_add(current_date(), interval " . (is_string($escapedDays) ? $escapedDays : '') .
    " day),'from CAMOS', ?, ?)";
...
function addBilling2(int $encounter, string $code_type, string $code, string $code_text, ?string $modifier = null, ?string $units = null, ?string $fee = null, array $justify = []): int
{
  ...
  //  can authorize as a feature in the future
    ...
    $session = SessionWrapperFactory::getInstance()->getActiveSession();
    return QueryUtils::sqlInsert($sql, [$session->get('encounter'), $code_type, $code, $code_text, $session->get('pid'), $authorized, $session->get('authUserID'), $session->get('authProvider'), $session->get('authUserID'), $modifier, $units, $fee, $justify_string]);
}
...
function process_commands(string &$string_to_process, array &$camos_return_data): int
{
  ...
  //needs to be here.
    if (preg_match("/\/\*\s*date_add\s*::\s*(.*?)\s*\*\//", $string_to_process, $matches)) {
        ...
        $days = $matches[1];
        $escapedDays = add_escape_custom($days);
        $query = "select date_format(date_add(date, interval " . (is_string($escapedDays) ? $escapedDays : '') . " day),'%W, %m-%d-%Y') as date from form_encounter where pid = ? and encounter = ?";
    ...
    if (preg_match("/\/\*\s*date_sub\s*::\s*(.*?)\s*\*\//", $string_to_process, $matches)) {
        ...
        $days = $matches[1];
        $escapedDays = add_escape_custom($days);
        $query = "select date_format(date_sub(date, interval " . (is_string($escapedDays) ? $escapedDays : '') . " day),'%W, %m-%d-%Y') as date from form_encounter where pid = ? and encounter = ?";
        $statement = QueryUtils::sqlStatementThrowException($query, [$session->get('pid'), $session->get('encounter')]);
        /** @var array{date: string}|false $result */
        if ($result = QueryUtils::fetchArrayFromResultSet($statement)) {
            $string_to_process = str_replace($to_replace, $result['date'], $string_to_process);
 ...
 //replace placeholders with values
    ...
    if ($results) {
        ...
        $name = $mname ? $fname . ' ' . $mname . ' ' . $lname : $fname . ' ' . $lname;
            ...
            $dob = $results['DOB'];
            $date = $results['date'];
            $age = (string) patient_age($dob, $date);
