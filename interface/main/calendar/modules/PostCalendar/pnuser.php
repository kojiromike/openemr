 ...
 * available from the module.
 */
function postcalendar_user_display($args): false|string
{
...
function postcalendar_user_search(): string
{
    ...
    $prepared = pnVarCleanFromInput('pc_keywords_andor');
    ...
    //modified 8/09 by BM to allow translation if applicable
        $cat_options .= '<option value="' . attr($category['id']) . sprintf('" %s>', $selected) . text(xl_appt_category($category['name'])) . "</option>";
    }
    ...
    if (!empty($submit) && strtolower($submit) === "listapps") {
        ...
        foreach ($keywords as $keyword) {
            if ($sqlKeywords !== '' && $sqlKeywords !== '0') {
                $sqlKeywords .= " " . escape_identifier($prepared, ['AND', 'OR'], true) . " ";
            }

            $sqlKeywords .= '(';
            $sqlKeywords .= "pd.lname LIKE '%" . add_escape_custom($keyword) . "%' OR ";
            $sqlKeywords .= "pd.fname LIKE '%" . add_escape_custom($keyword) . "%' OR ";
            $sqlKeywords .= "u.lname LIKE '%" . add_escape_custom($keyword) . "%' OR ";
            $sqlKeywords .= "u.fname LIKE '%" . add_escape_custom($keyword) . "%' OR ";
            $sqlKeywords .= "a.pc_title LIKE '%" . add_escape_custom($keyword) . "%' OR ";
            $sqlKeywords .= "a.pc_hometext LIKE '%" . add_escape_custom($keyword) . "%' OR ";
            $sqlKeywords .= "a.pc_location LIKE '%" . add_escape_custom($keyword) . "%'";
            $sqlKeywords .= ') ';
        }
        ...
        if (isset($ProviderID) && $ProviderID != "") { // && $ProviderID != "_ALL_") {
            ...
            $searchargs['provider_id'][] = $ProviderID;
        }
