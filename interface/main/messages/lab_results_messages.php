...
function lab_results_messages($set_pid, $rid, $provider_id = ""): void
{
    ...
    if ($provider_id != "") {
        $where = "AND id = ?";
        $sqlBindArray[] = $provider_id;
    }

    // Get all active users.
    $recordset = sqlStatement('select id, username from users where username != \'\' AND active = \'1\' ' . $where, $sqlBindArray);
    ...
    foreach ($result as $user_detail) {
        ...
        if ($thisauth) {
            ...
            $note = sprintf("Patient %s's lab results have arrived. Please click %s to review them.<br/>", $pname, $link);
