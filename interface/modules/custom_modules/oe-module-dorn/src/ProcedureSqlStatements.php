...
class ProcedureSqlStatements
{
    ...
    {
        return sqlQuery("SELECT * FROM form_vitals v join forms f on f.form_id=v.id WHERE f.pid=? and f.encounter=? ORDER BY v.date DESC LIMIT 1", [$pid, $encounter]);
    }

    public static function getSpecimen($procedureCode)
    {
        return sqlQuery("SELECT specimen FROM procedure_type WHERE procedure_code=?", [$procedureCode]);
    }
