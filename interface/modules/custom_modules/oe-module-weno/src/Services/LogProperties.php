...
class LogProperties
{
     ...
     * @var false|string
     */
    private string|bool $weno_admin_password;
    ...
    public function fetchLogSyncDates(): array
    {
        ...
        if ($lastSync < $from) {
            ...
            if ($from < $sevenDaysAgo) {
                ...
                $wenoLogService->insertWenoLog("Sync Report (May be first sync)", $status);
            }
        }
    ...
    public function logSync($tasked = 'background'): bool
    {
        ...
        $wenoLogService->insertWenoLog("Sync Report", "Sent a Sync Request", $urlOut);
        ...
        if ($statusCode == 200) {
            ...
            $isError = $wenoLogService->scrapeWenoErrorHtml($rpt);
            if ($isError['is_error']) {
                ...
                if ($tasked == 'background') {
                    $wenoLogService->insertWenoLog("Sync Report", $error);
                    return false;
                }
                ...
                $wenoLogService->insertWenoLog("Sync Report", "Invalid Prescriber Credentials Hint:see previous errors.");
                echo(js_escape($error));
                return false;
            }
        } else {
            ...
            error_log(sprintf('Prescription download failed: errorLogEscape(%s)', $statusCode));
            $wenoLogService->insertWenoLog("Sync Report", 'Failed http_error_' . $statusCode);
            return false;
        }

        if (file_exists($this->rxsynclog)) {
            $logImportBuild = new LogImportBuild();
            $rtn = $logImportBuild->buildPrescriptionInserts();
            if ($rtn != true) {
                return false;
            }
        } else {
