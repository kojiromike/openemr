...
{
    public function generateSelectQueryForTableFromMetadata(ExportTableDefinition $exportTableDefinition, \SimpleXMLElement $metaNode): void
    {
        ...
        $xpathColumnsToExclude = $metaNode->xpath("//table[@name='" . $exportTableDefinition->table . "']/column[@exclude='true']");
        if (!empty($xpathColumnsToExclude)) {
            $columns = $exportTableDefinition->getColumnNames();
            $hashMap = array_combine($columns, $columns);
            foreach ($xpathColumnsToExclude as $xpathColumnToExclude) {
                $columnName = (string)($xpathColumnToExclude->attributes()['name']) ?? "";
                if ($columnName !== '' && $columnName !== '0' && !empty($hashMap[$columnName])) {
                    $hashMap[$columnName] = "null as " . $columnName;
                }
            }

            $selectClause = implode(',', $hashMap);
            $exportTableDefinition->setSelectClause($selectClause);
        }
    }
}
