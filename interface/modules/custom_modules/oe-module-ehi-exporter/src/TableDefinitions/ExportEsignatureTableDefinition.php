...
class ExportEsignatureTableDefinition extends ExportTableDefinition
{
    ...
    public function getRecords(): array
    {
        ...
        $sql = sprintf('SELECT %s FROM `', $selectQuery) . self::TABLE_NAME . "` WHERE `table`='forms' AND `tid` IN (select `id` FROM `forms` WHERE `pid` IN ( "
            . str_repeat("?, ", $patientIdsCount - 1) . "? ) )";
        $records = QueryUtils::fetchRecords($sql, $patientPids);

        $encounterSql = sprintf('SELECT %s FROM `', $selectQuery) . self::TABLE_NAME . "` WHERE `table`='form_encounter' AND `tid` IN (select `encounter` FROM `form_encounter` WHERE `pid` IN ( "
            . str_repeat("?, ", $patientIdsCount - 1) . "? ) )";
        $encounterRecords = QueryUtils::fetchRecords($encounterSql, $patientPids);
        return array_merge($records, $encounterRecords);
    }
}
